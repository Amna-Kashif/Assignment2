...
// // show mobile brands in first dropdown  
var mobileList = [];
var mobileTypes = [];

// Assuming 'mobiles' is an object with brand names as keys and models as values.
for (let brand in mobiles) {
  let mobileBrandObj = {
    mobileBrands: brand
  };
  mobileList.push(mobileBrandObj);
}

function print(){
let insertValue = '';
for (let key of mobileList) {
  insertValue += ` <option value="${key.mobileBrands}">${key.mobileBrands}</option> `;
}
document.getElementById('mainDropdown').innerHTML += insertValue;
}
print()

function populateSecondDropdown() {
  mobileTypes = [];
  let selectedMobile = document.getElementById('mainDropdown').value;
  let mobileModels = extractMobiles(selectedMobile);
  iterateDetails(mobileModels);
}

function extractMobiles(selectedMobile) {
  for (let mobileBrands in mobiles) {
    if (selectedMobile === mobileBrands) {
      return mobiles[mobileBrands]; // Return models of the selected brand
    }
  }
}

function iterateDetails(models) {
  // Reset the second dropdown before populating
  document.getElementById('secondDropdown').innerHTML = '';

  // Populate the second dropdown with mobile models
  mobileTypes = [];
  for (let key in models) {
    let mobileBrandsObj = {
      mobileBrands: key
    };
    mobileTypes.push(mobileBrandsObj);
  }

  let insertValue = '';
  for (let element of mobileTypes) {
    insertValue += `
      <option value="${element.mobileBrands}">${element.mobileBrands}</option>
    `;
  }

  document.getElementById('secondDropdown').innerHTML += insertValue;
}


 // // Display features of the selected mobile model
 function searchFeatures() {
  // Get the selected brand and model from the dropdowns
  let selectedBrand = document.getElementById('mainDropdown').value;
  let selectedModel = document.getElementById('secondDropdown').value;
  let showDetails = document.getElementById('features');
   // Clear the previous content
  showDetails.innerHTML = '';
  if (selectedBrand && selectedModel) {
      // Extract the mobile details for the selected brand and model
      let mobileDetails = mobiles[selectedBrand][selectedModel];
      // If the model exists in the 'mobiles' object, extract its features
      if (mobileDetails && mobileDetails.specialFeatures) {
          let features = mobileDetails.specialFeatures;
          // Create the HTML structure to display features
          let featuresHTML = `<h3>Features of ${selectedModel}:</h3><p>`; 
          // Loop through the features array and display each feature
          for (let feature of features) {
              featuresHTML += `<p>${feature}</p>`;
          }
          featuresHTML += `</p>`;
          // Insert the features
          showDetails.innerHTML += featuresHTML;
      } else {
          // In case there are no features or if the model doesn't exist
          showDetails.textContent = JSON.stringify(model,null,2)
          showDetails.innerHTML = "<p>No features available for this model.</p>";
      }
  } else {
      // Clear the features if no brand or model is selected
      showDetails.innerHTML = '';
  }
}